...
class StoreManager:
    ...
    async def close(self):
        if self.store:
            # AsyncPostgreStore 通常不需要显式 close store 对象
            # 但需要关闭底层的 connection pool
            pass

        if self.pool:
            await self.pool.close()
            app_logger.info("Connection Pool 已经关闭")
...
class UserMemoryService:
    ...
    async def save_user_profile(self, user_id: str, profile: UserProfile):
        ...
        await self.store.aput(
            namespace=("user_profiles", user_id), # 数据属于哪个逻辑空间，一般是一个tuple
            key="profile",
