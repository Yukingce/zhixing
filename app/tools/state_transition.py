...
"""
departure_city / departure_date / travel_days / budget_min / budget_max / travel_styles 
都是必填参数（没有默认值）。这意味着 LLM 若信息没收集全，在结构上就无法调用这个工具（缺参会被拒绝），
于是只能继续追问。这才是"信息不完整就继续问"最硬的实现。

没说完的需求不会丢，靠的是 checkpointer。
下一轮用户再说话，LangGraph 先从库里把这个 thread_id 的历史 messages 恢复出来，再交给 LLM。
"""
