...
from app.rag.documents_loader import DocumentManager
...
# ============== 全局 RAG 管道实例（懒加载 + 单例） ==============
#
# 这里采用「懒加载（Lazy Loading）+ 单例（Singleton）」模式：
#
#   - 懒加载：创建 RAG 管道很昂贵（要加载文档、切分文本、加载/构建向量库、
#     连接 LLM），所以不在一开始 import 时创建，而是等第一次真正检索时才初始化。
#   - 单例：整个模块只维护这一份实例，初始化一次后永久复用，
#     避免每次检索都重复加载文档和向量库。
#
# 两个变量都是「全局缓存槽位」，初始为 None 表示“尚未初始化”，
# 后续由 _get_rag_pipeline() 在第一次调用时填充。

_rag_pipeline: Optional[AdvancedRAGPipeline] = None      # RAG 管道实例缓存
_parent_splitter: Optional[AdvancedParentDocumentSplitter] = None  # 父文档切分器缓存


async def _get_rag_pipeline() -> AdvancedRAGPipeline:
    """
    获取或初始化全局 RAG 管道实例（懒加载 + 单例模式）。

    懒加载逻辑：只有当全局变量 _rag_pipeline 仍为 None（即从未初始化过）时，
    才执行下方昂贵的初始化流程；一旦初始化完成，后续所有调用都直接命中缓存，
    返回同一个实例，不再重复加载文档 / 构建向量库。
    """
    global _rag_pipeline, _parent_splitter

    # 懒加载判断：首次调用时 _rag_pipeline 为 None，走初始化分支；之后直接跳过
    if _rag_pipeline is None:
        app_logger.info("🔧 初始化 RAG 管道...")

        # 以下为一次性初始化流程，仅在首次调用时执行：
        # 1. 加载文档
    ...
    # 返回缓存实例：首次调用是刚初始化的新实例，之后都是同一个单例
